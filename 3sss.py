...
def test_mixtures_three_elements(element1, element2, element3):
    """
    Test all possible concentrations for a mixture of three elements.
    """
    step = 0.1  # Increment step for concentrations
    concentrations = np.arange(0, 1 + step, step)
    results = []

    for c1 in concentrations:
        for c2 in concentrations:
            c3 = 1 - c1 - c2  # Complementary concentration
            if c3 < 0 or c3 > 1:  # Skip invalid combinations
                continue
...
if __name__ == "__main__":
    ...
    if (element1_name not in element_properties 
        ...
        # Display the results
        print(f"\nConcentration of {element1_name} (%) | Concentration of {element2_name} (%) | Concentration of {element3_name} (%) | Shear Modulus (G) | Solid Solution Strength (MPa)")
        print("-" * 100)
        for result in results:
            print(f"{result['c1']*100:>25.1f} | {result['c2']*100:>25.1f} | {result['c3']*100:>25.1f} | {result['shear_modulus']:.6f} | {result['solid_solution_strength']:.6f}")
